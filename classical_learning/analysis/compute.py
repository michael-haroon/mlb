...
def set_parallel_mode(enabled: bool):
    """When outer orchestrator runs (target × test) in parallel, each worker
    sets this True so inner joblib calls use n_jobs=1."""
...
def get_n_jobs() -> int:
    ...
    Returns 1 when parallel_mode is active (worker process should not
    spawn its own parallel pool).
    """
    if _PARALLEL_MODE:
        return 1
