...
class SlidingWindowYearCV:
    """Sliding-window cross-validation: train on last K years only.

    Captures temporal dynamics invisible to expanding windows — features
    whose importance has changed over time (emerged or decayed) are surfaced
    rather than averaged away by long history.
    """
    ...
    def split(self, X, y=None, groups=None):
        ...
        for test_year in self.unique_years:
            ...
            eligible_years = [s for s in self.unique_years
                             if s < test_year and s not in self.skip_seasons]

            if len(eligible_years) < self.window_size:
                continue

            train_years = eligible_years[-self.window_size:]
...
def make_year_cv(years: pd.Series, mode: str = "expanding"):
    """Factory for year-based CV splitters.

    Args:
        years: season Series
        mode: "expanding" or "sliding_3" (sliding window of size 3)
    """
    if mode == "expanding":
        return ExpandingWindowYearCV(years)
    elif mode == "sliding_3":
        return SlidingWindowYearCV(years, window_size=3)
    else:
        raise ValueError(f"Unknown cv_mode: {mode!r}. Use 'expanding' or 'sliding_3'.")
...
def _mda_one_fold(clf_params, X_tr_vals, X_te_vals, y_tr_vals, y_te_vals,
                  ...
                  n_repeats=1, return_repeat_sd=False):
    ...
    n_repeats: number of independent permutations per feature (averaged).
    """
    from sklearn.tree import DecisionTreeClassifier, DecisionTreeRegressor
    from sklearn.ensemble import BaggingClassifier, BaggingRegressor
    ...
    for col in col_names:
        rep_scores = np.empty(n_repeats)
        for rep in range(n_repeats):
            rng_rep = np.random.default_rng(seed * 10000 + rep)
            X_perm = X_te.copy()
            X_perm[col] = rng_rep.permutation(X_perm[col].values)
            if is_regression:
                pred_perm = fit.predict(X_perm)
                rep_scores[rep] = r2_score(y_te, pred_perm)
            else:
                prob_perm = _align_proba(fit.predict_proba(X_perm), fit.classes_, ll_labels)
                if scoring == "log_loss":
                    rep_scores[rep] = -log_loss(y_te, prob_perm, labels=ll_labels)
                else:
                    rep_scores[rep] = roc_auc_score(y_te, prob_perm[:, 1])
        perm_fold[col] = rep_scores.mean()
        if return_repeat_sd:
            perm_fold_sd[col] = rep_scores.std(ddof=1) if n_repeats > 1 else 0.0

    if return_repeat_sd:
        return base_score, perm_fold, perm_fold_sd
...
def feat_imp_mda(clf,
                 ...
                 n_repeats: int = 1,
                 return_repeat_sd: bool = False,
                 cv_mode: str = "expanding") -> tuple:
    """Mean Decrease Accuracy via year-based CV.

    Shuffles one feature at a time and measures accuracy drop.
    Parallelised over folds via joblib with multiprocessing backend.

    Parameters:
        n_repeats: number of independent permutations averaged per fold.
            Reduces MC noise by sqrt(n_repeats) without retraining.
        return_repeat_sd: if True, also return within-fold SD across repeats.
        cv_mode: "expanding" (all prior years) or "sliding_3" (last 3 years).

    Returns:
        summary: DataFrame(index=feature, columns=[mean, std])
        raw:     DataFrame(index=fold, columns=features) — per-fold importance
        repeat_sd_raw: (only if return_repeat_sd) DataFrame of within-fold SDs
    """
    cv = make_year_cv(years, mode=cv_mode)
    ...
    with blas_limit(1):
        results = Parallel(n_jobs=n_outer, backend="multiprocessing")(
            delayed(_mda_one_fold)(
                ...
                n_repeats=n_repeats,
                return_repeat_sd=return_repeat_sd,
            )
    ...
    if return_repeat_sd:
        sd_records = {}
        for col in X.columns:
            sd_records[col] = np.array([r[2][col] for r in results])
        repeat_sd_raw = pd.DataFrame(sd_records)
        return result.sort_values("mean", ascending=False), raw, repeat_sd_raw
...
def _desub_mda_one_task(feat_idx, other_idxs, X_vals, X_tr_idx, X_te_idx,
    ...
    n_repeats: number of independent permutations averaged (reduces MC noise by sqrt(n)).
    """
    from sklearn.tree import DecisionTreeClassifier, DecisionTreeRegressor
    from sklearn.ensemble import BaggingClassifier, BaggingRegressor
    ...
    try:
        ...
        for rep in range(n_repeats):
            rng_rep = np.random.default_rng(seed * 10000 + rep)
            X_te_perm = X_te.copy()
            X_te_perm[:, 0] = rng_rep.permutation(X_te_perm[:, 0])

            if regression:
                rep_scores[rep] = _r2(y_te, fit.predict(X_te_perm))
            else:
                prob_perm = _align_proba(fit.predict_proba(X_te_perm), fit.classes_, ll_labels)
                rep_scores[rep] = -log_loss(y_te, prob_perm, labels=ll_labels)

        mean_imp = base_score - rep_scores.mean()
        if return_repeat_sd:
            imp_sd = rep_scores.std(ddof=1) if n_repeats > 1 else 0.0
            return mean_imp, imp_sd
        return mean_imp
    except Exception:
        return None
...
def feat_imp_desub_mda(X: pd.DataFrame,
    ...
    Parameters:
        n_repeats: number of independent permutations averaged per task.
        return_repeat_sd: if True, also return within-fold SD across repeats.
    ...
    sd_records = {col: [None] * n_folds for col in col_names} if return_repeat_sd else None
    for (feat, fi, *_), result in zip(tasks, results):
        if result is None:
            continue
        if return_repeat_sd:
            raw_records[feat][fi] = result[0]
            sd_records[feat][fi] = result[1]
        else:
            raw_records[feat][fi] = result
    ...
    if return_repeat_sd:
        final_sd = {}
        for col in col_names:
            sds = [s for s in sd_records[col] if s is not None]
            if sds:
                final_sd[col] = sds
        max_folds_sd = max(len(v) for v in final_sd.values()) if final_sd else 0
        repeat_sd_raw = pd.DataFrame(
            {col: vals + [np.nan] * (max_folds_sd - len(vals))
             for col, vals in final_sd.items()}
        )
        return summary.sort_values("mean", ascending=False), raw, repeat_sd_raw
...
def _sfi_one_task(col_idx, X_col_vals, X_tr_idx, X_te_idx,
    ...
    if regression:
        ...
        # sfi_class_weight=None for targets with >5x imbalance — see config.SFI_CLASS_WEIGHT_OVERRIDES
        base = DecisionTreeClassifier(
            criterion="entropy", max_features=1,
            class_weight=sfi_class_weight, min_weight_fraction_leaf=0.02,
        )
        clf = BaggingClassifier(
            estimator=base, n_estimators=n_estimators,
            max_features=1.0, max_samples=1.0, oob_score=False,
...
def feat_imp_sfi(clf,
                 ...
                 sample_weight: pd.Series = None,
                 regression: bool = False,
                 sfi_class_weight: "str | None" = "balanced",
                 cv_mode: str = "expanding") -> tuple:
...
def _onc_flat(corr: pd.DataFrame,
    ...
    log.info(f"    {label}ONC flat: {len(combos)} combos (k=2..{max_clusters}, "
             f"n_init={n_init}, n={n} features, n_jobs={n_jobs})")
    with blas_limit(1):
        results = Parallel(n_jobs=n_jobs, backend="loky")(
            delayed(_onc_one_combo)(X_vals, k, seed) for k, seed in combos
        )
    log.info(f"    {label}ONC flat done: {sum(1 for r in results if r is not None)}/{len(combos)} valid fits")
...
def onc_cluster(corr: pd.DataFrame,
    ...
    """
    log.info("  ONC: initial flat pass...")
    partition = _onc_flat(corr, max_clusters=max_clusters, n_init=n_init, label="initial ")
    if partition is None or len(partition) <= 1:
        return {0: list(corr.columns)}

    current_quality = _global_mean_silhouette(corr, partition)
    log.info(f"  ONC initial: {len(partition)} clusters, silhouette={current_quality:.4f}")
    ...
    while improved:
        ...
        n_eligible = sum(1 for c in sorted_cids if len(partition[c]) >= 4)
        log.info(f"  ONC divisive round {round_num}: {len(partition)} clusters, "
                 f"{n_eligible} eligible for subdivision, silhouette={current_quality:.4f}")
        ...
        for cid in sorted_cids:
            ...
            sub = _onc_flat(sub_corr, max_clusters=max_clusters, n_init=n_init,
                            label=f"r{round_num}.{attempt} split c{cid}({len(members)}feat) ")
            if sub is None or len(sub) <= 1:
                log.info(f"    r{round_num}.{attempt} cluster {cid} ({len(members)} feat): no valid split")
                continue
            ...
            if candidate_quality > current_quality:
                log.info(f"    r{round_num}.{attempt} cluster {cid} ({len(members)} feat): "
                         f"ACCEPTED → {len(candidate)} clusters, "
                         f"silhouette {current_quality:.4f} → {candidate_quality:.4f}")
                partition = candidate
                ...
                log.info(f"    r{round_num}.{attempt} cluster {cid} ({len(members)} feat): "
                         f"rejected (silhouette {candidate_quality:.4f} <= {current_quality:.4f})")

    log.info(f"  ONC converged: {len(partition)} clusters, silhouette={current_quality:.4f}")
    return partition
...
# ─────────────────────────────────────────────────────────────────────────────
#  Feature gate: filter_features_v2
# ─────────────────────────────────────────────────────────────────────────────
...
_ALL_METHODS = {"SFI", "DESUB_MDA", "PCA_MDA", "RESID_MDA", "MDI"}
...
def filter_features_v2(
    ...
    """Feature gate with conservative union.

    All 5 per-feature methods (MDI, SFI, DESUB_MDA, PCA_MDA, RESID_MDA)
    always participate in gating and ranking.
    ...
    Conservative union: REJECT only if ALL per-feature methods reject.
    """
    feat_to_cluster = {}
    for cid, members in clusters.items():
        for m in members:
    ...
    log.info(f"    All methods participate: {sorted(_ALL_METHODS)}")
    ...
    for feat in sorted(all_features):
        ...
        if row["cluster_vetoed"]:
            ...
            if not decision_list:
                ...
                would_reject = True
            elif _mdi_only_supporter(decisions):
                would_reject = True
    ...
    # Composite rank from all methods
    rank_cols = [f"{m.lower()}_rank" for m in _ALL_METHODS
...
def run_all_importance(X: pd.DataFrame,
                       ...
                       sfi_class_weight: "str | None" = "balanced") -> dict:
    ...
    # ── Step 10: Algorithmic filtering (all methods, conservative union) ─
    log.info("10/10  Algorithmic filtering (all methods, conservative union)...")
