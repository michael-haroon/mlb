...
def _classify_feature(row: pd.Series) -> str:
    ...
    # Residualized signal only: unique cross-cluster orthogonal predictive power.
    # Linear models can exploit this even without PCA support.
    if resid and not pca and not mdi and not sfi:
        return "linear_only"
...
def route_features(filter_report: pd.DataFrame) -> dict:
    ...
    Always uses evidence-group routing: features are classified by their
    pass/fail pattern across importance methods, then routed to model
    families based on architectural compatibility.
