...
def main() -> None:
    ...
    imp_parser.add_argument("--cfi-mdi-only", action="store_true", default=False,
                             help="Run ONLY CFI-MDI (de Prado per-tree aggregation). "
                                  "Skips all OOS methods. Requires --cluster-map.")
    imp_parser.add_argument("--cluster-map", default=None,
                             help="Path to precomputed cluster_map.json (for --cfi-mdi-only)")
...
def _run_importance(args):
    ...
    if args.cfi_mdi_only:
        from .analysis.feature_importance import run_cfi_mdi_only
        from .strategy.data import compute_temporal_weights, load_features
        from .strategy.config import TARGETS_CLASSIFICATION
        import json as _json

        if not args.cluster_map:
            raise SystemExit("--cfi-mdi-only requires --cluster-map <path>")

        cluster_map_path = Path(args.cluster_map)
        with open(cluster_map_path) as f:
            clusters = {int(k): v for k, v in _json.load(f).items()}

        for target in targets:
            regression = target not in TARGETS_CLASSIFICATION
            X, y, seasons, _ = load_features(Path(args.features), target, args.data_mode)
            ...
            result["cfi_mdi_cluster"].to_csv(output_dir / "importance_cfi_mdi_cluster.csv")
            result["cfi_mdi_per_feature"].to_csv(output_dir / "importance_cfi_mdi_per_feature.csv")
            result["cfi_mdi_raw"].to_csv(output_dir / "importance_cfi_mdi_raw.csv")
            results[target] = {"status": "done", "n_clusters": len(result["cfi_mdi_cluster"])}

        print(json.dumps(results, indent=2))
        return

    from .analysis.run import run_importance_parallel
