...
def build_ensemble(
    ...
    if task == "classification":
        ...
        # Filter by RMSE quality before greedy selection. Without this,
        ...
        rmse_scores = {
            name: metrics[name]["rmse"]
            for name in oof_matrix
            if name in metrics and "rmse" in metrics[name]
        }
        if rmse_scores:
            best_rmse = min(rmse_scores.values())
            threshold = best_rmse * metric_tolerance
            viable = {name: rmse_scores[name] for name in rmse_scores if rmse_scores[name] <= threshold}
            dropped = [n for n in oof_matrix if n not in viable]
            if dropped:
                log.info(f"  Regression RMSE quality filter (≤ {threshold:.4f}): dropped {dropped}")
        else:
...
def _optimize_weights(
    ...
    if task == "classification":
        ...
        def objective(w):
            blended = pred_matrix @ w
            return np.mean((y_true - blended) ** 2)
...
def fit_and_save_ensemble(
    ...
    for family in members:
        ...
        # Resolve feature set from saved params first (guarantees train/refit alignment).
        # feature_columns in the params JSON is exactly what OOF used — the ensemble
        # weight was computed from that feature set, so the refit must use the same.
        # Falling back to get_feature_set() would use old capped routing that ignores
        # per-family sizing and MDA regime, producing mismatched feature counts.
        importance_features = None
        if "feature_columns" in params_data:
            available = set(X.columns)
            importance_features = [f for f in params_data["feature_columns"] if f in available]
