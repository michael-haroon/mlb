...
#### `register_game(game_pk, pregame_prior, weather_context=None, venue_id=None, game_hour_utc=None)`

Initialize inference state for a new live game. If `weather_context` is omitted and
`venue_id`/`game_hour_utc` are supplied, the engine fetches the [4, 22] weather tensor
via `fetch_live_weather`; a fetch failure registers the game with a warning rather
than dropping it.

**Parameters**:
- `game_pk` (int): MLB game primary key
- `pregame_prior` (PregamePrior): Pregame model output for this game
- `weather_context` (Optional[torch.Tensor]): [4, 22] weather tensor; fetched if omitted
- `venue_id` (Optional[int]): stadium venue_id, required to fetch weather
...
engine.register_game(12345, prior, venue_id=3313, game_hour_utc=game_hour)
```
