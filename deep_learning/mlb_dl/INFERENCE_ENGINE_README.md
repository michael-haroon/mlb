...
```bash
conda run -n pred pytest deep_learning/mlb_dl/tests/test_inference_engine.py -v
```
...
1. **Ingestion Layer** (`deep_learning/mlb_dl/ingestion.py`)
...
2. **Model Training** (`deep_learning/mlb_dl/train.py`)
   ...
   - Save checkpoint to `deep_learning/mlb_dl/checkpoints/best_model.pt`
...
**Key Methods**:
- `register_game(game_pk, pregame_prior, weather_context=None, venue_id=None, game_hour_utc=None)` — Initialize state; fetches the weather tensor when venue/hour are supplied
...
**Run**:
```bash
conda run -n pred pytest deep_learning/mlb_dl/tests/test_inference_engine.py -v -s
```
...
4. **Implement ingestion layer** (`deep_learning/mlb_dl/ingestion.py`):
   - WebSocket client for MLB GUMBO API
   - Parse play-by-play into `PitchEvent` objects
...
All architectural decisions are documented with WHY rationale. All unvalidated constants are marked as placeholders. Models have been trained (`train_unified.py`, corrected-population run 2026-08-29); the remaining gap is production wiring — nothing calls `register_game` from the live daemon/runner yet.
