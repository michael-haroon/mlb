...
class MarketDeriver:
    ...
    def p_team_total_over(
        ...
        # int(X.5) == int(X.0) == X for both half-point and integer lines,
        # so both cases reduce to the same formula.
        return 1.0 - cdf[int(threshold)]
    ...
    def derive_first_5_markets(
        ...
        we need P(runs in innings [current+1 ... 5]). Linear scaling of mu by innings
        fraction; alpha scaled identically to preserve the variance-to-mean ratio.
        ...
        # Scale alpha with mu to preserve variance-to-mean ratio under time-subsetting.
        # Consistent with scale_to_first_5(); see its WHY comment for derivation.
        alpha_h_f5 = alpha_h_remaining_full * scale_factor
        alpha_a_f5 = alpha_a_remaining_full * scale_factor
...
def scale_to_first_5(
    ...
    alpha is scaled with mu to preserve the variance-to-mean ratio (see implementation WHY comment).
