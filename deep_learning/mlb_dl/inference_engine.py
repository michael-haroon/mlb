...
from .weather_context import fetch_live_weather

log = logging.getLogger(__name__)

# The calibrated CF azimuths the weather tensor was *trained* with. Dims 4-5
# (wind_out_cf, wind_cross) are the 10m wind rotated into park coordinates, so a
# different azimuth table is a different feature — at a park facing south the 0°
# fallback flips the sign of wind_out_cf.
_PARK_AZIMUTHS_URI = (
    "s3://mlb-265753586044-us-east-1-an/classical_learning/artifacts/features/"
    "park_azimuths.json"
)


def _load_park_azimuths(uri: str) -> dict[int, float]:
    """Load park_azimuths.json from S3 or a local path. Returns {} on failure."""
    import json
    ...
    try:
        ...
        log.warning(
            f"Could not load park_azimuths from {uri}: {exc} — dims 4-5 "
            f"(wind_out_cf, wind_cross) will fall back to a 0° rotation, which "
...
class LiveInferenceEngine:
    ...
    def __init__(
        ...
        log.info(f"Loaded {len(self.park_azimuths)} park CF azimuths")
    ...
    def register_game(
        ...
        weather_context: Optional[torch.Tensor] = None,
        venue_id: Optional[int] = None,
        game_hour_utc: Optional[Any] = None,
    ) -> None:
        """Initialize inference state for a new live game.

        Args:
            weather_context: [4, 22] tensor. If omitted and venue_id/game_hour_utc
                are given, it is fetched here from the live ECMWF forecast parquet.
            venue_id: stadium venue_id, required to fetch weather.
            game_hour_utc: first-pitch hour (UTC), required to fetch weather.

        Weather is fetched once per game rather than per pitch: the tensor spans
        ...
        if self._asof_mode and venue_id is not None and game_hour_ts is not None:
            ...
            try:
                arr = fetch_live_weather(
                    ...
                    park_azimuths=self.park_azimuths,
                )
                weather_context = torch.from_numpy(arr)
            except Exception:
                # Weather is one of several context blocks. Losing it degrades the
                # forward pass; dropping the game loses live pricing entirely.
                log.warning(
                    f"Game {game_pk}: live weather fetch failed (venue={venue_id}) "
                    f"— registering without weather context",
                    exc_info=True,
                )
        elif weather_context is None:
            log.warning(
                f"Game {game_pk}: no venue_id/game_hour_utc supplied — the model's "
                f"weather branch will be skipped for this game"
            )
