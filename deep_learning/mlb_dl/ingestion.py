...
class PitchEvent:
    ...
    def __init__(self, raw_event: dict, game_state: dict):
        ...
        # Catcher identity (fielder_2 in Statcast schema)
        self.catcher_id: int = game_state.get("catcher_id", 0)

        # Whether this is an actual pitched ball vs. action/pickoff/no_pitch
        self.is_pitch: bool = raw_event.get("isPitch", True)
        self.event_type_raw: str = raw_event.get("type", "pitch")
    ...
    def _extract_new_pitches(self, gumbo_data: dict, game_state: dict) -> list[PitchEvent]:
        ...
        if current_count > self._last_pitch_count:
            ...
            for play in all_plays[self._last_pitch_count:]:
                ...
                # Each play can have multiple events (pitches + actions)
                for event in play_events:
                    try:
                        ...
                        logger.warning(
                            f"[WS {self.game_pk}] Failed to parse event: {exc}"
                        )
