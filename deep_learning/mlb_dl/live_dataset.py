...
class LiveHANDataset(Dataset):
    ...
    def __init__(
        ...
        # Convert dates in place (no copies)
        game_targets = game_targets.copy()
        game_targets["game_date"] = pd.to_datetime(game_targets["game_date"], errors="coerce")
        pregame_features["game_date"] = pd.to_datetime(pregame_features["game_date"], errors="coerce")
        ...
        # Exclude structural outlier seasons
        ...
        if SKIP_SEASONS:
            ...
            if "season" in pitch_sequences.columns:
                pitch_sequences = pitch_sequences[~pitch_sequences["season"].isin(SKIP_SEASONS)]
            if "season" in pregame_features.columns:
                pregame_features = pregame_features[~pregame_features["season"].isin(SKIP_SEASONS)]
        ...
        # Build per-game lookup (small — one row per game)
        self.target_by_game = {int(row.game_pk): row for row in targets.itertuples(index=False)}
        self.pregame_by_game = {
            int(row.game_pk): row for row in pregame_features.itertuples(index=False)
            if int(row.game_pk) in self.target_by_game
        }

        # Memory-efficient pitch storage: one sorted DataFrame + offset index
        valid_games = set(self.target_by_game.keys())
        pitch_seqs = pitch_sequences[pitch_sequences["game_pk"].isin(valid_games)]
        pitch_seqs = pitch_seqs.sort_values(["game_pk", "pitch_sequence_index"]).reset_index(drop=True)
        self._pitches = pitch_seqs  # Single reference, no copy

        # Build game_pk → (start_row, end_row) offset index
        self._game_offsets = {}
        game_pks = pitch_seqs["game_pk"].to_numpy()
        if len(game_pks) > 0:
            changes = np.where(game_pks[1:] != game_pks[:-1])[0] + 1
            starts = np.concatenate([[0], changes])
            ends = np.concatenate([changes, [len(game_pks)]])
        ...
        self.samples = []
        for game_pk, (start, end) in self._game_offsets.items():
            n_pitches = end - start
            if n_pitches == 0:
                continue

            positions = list(range(0, n_pitches, stride))
            if positions[-1] != n_pitches - 1:
                positions.append(n_pitches - 1)
        ...
        # Sample weights (game-level decay)
    ...
    def __getitem__(self, idx: int) -> dict:
        ...
        # Get pitch sequence prefix from offset index (zero-copy slice)
        start_row, _ = self._game_offsets[game_pk]
        seq = self._pitches.iloc[start_row:start_row + end_pos + 1]
