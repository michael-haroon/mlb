...
def main() -> None:
    ...
    # Learning curves: pregame CNN at multiple data fractions
    lc_pregame = sub.add_parser("learning-curves-pregame", help="Train pregame CNN at multiple data fractions")
    lc_pregame.add_argument("--feature-store", required=True)
    lc_pregame.add_argument("--output", required=True)
    lc_pregame.add_argument("--fractions", type=float, nargs="+", default=[0.10, 0.25, 0.50, 0.75, 1.0])
    lc_pregame.add_argument("--history-length", type=int, default=20)
    lc_pregame.add_argument("--min-history", type=int, default=5)
    lc_pregame.add_argument("--epochs", type=int, default=20)
    lc_pregame.add_argument("--batch-size", type=int, default=64)
    lc_pregame.add_argument("--learning-rate", type=float, default=5e-4)
    lc_pregame.add_argument("--weight-decay", type=float, default=1e-4)
    lc_pregame.add_argument("--hidden-dim", type=int, default=128)
    lc_pregame.add_argument("--dropout", type=float, default=0.2)
    lc_pregame.add_argument("--time-decay-lambda", type=float, default=0.003)
    lc_pregame.add_argument("--patience", type=int, default=5)
    lc_pregame.set_defaults(func=_cmd_learning_curves_pregame)

    # Flat MLP on full classical feature store (898 features, no sequence)
    fit_full = sub.add_parser("fit-pregame-full", help="Train residual MLP on full classical game_features.parquet")
    fit_full.add_argument("--game-features", required=True, help="Path to game_features.parquet (classical feature store)")
    fit_full.add_argument("--output", required=True)
    fit_full.add_argument("--epochs", type=int, default=50)
    fit_full.add_argument("--batch-size", type=int, default=128)
    fit_full.add_argument("--learning-rate", type=float, default=1e-3)
    fit_full.add_argument("--weight-decay", type=float, default=1e-3)
    fit_full.add_argument("--hidden-dim", type=int, default=256)
    fit_full.add_argument("--n-blocks", type=int, default=4)
    fit_full.add_argument("--dropout", type=float, default=0.3)
    fit_full.add_argument("--patience", type=int, default=10)
    fit_full.set_defaults(func=_cmd_fit_pregame_full)

    # Learning curves: live HAN at multiple data fractions
    lc_live = sub.add_parser("learning-curves-live", help="Train live HAN at multiple data fractions")
    lc_live.add_argument("--feature-store", required=True)
    lc_live.add_argument("--output", required=True)
    lc_live.add_argument("--fractions", type=float, nargs="+", default=[0.10, 0.25, 0.50, 0.75, 1.0])
    lc_live.add_argument("--epochs", type=int, default=30)
    lc_live.add_argument("--batch-size", type=int, default=32)
    lc_live.add_argument("--learning-rate", type=float, default=3e-4)
    lc_live.add_argument("--weight-decay", type=float, default=1e-4)
    lc_live.add_argument("--d-model", type=int, default=128)
    lc_live.add_argument("--n-heads", type=int, default=4)
    lc_live.add_argument("--dropout", type=float, default=0.1)
    lc_live.add_argument("--max-seq-len", type=int, default=350)
    lc_live.add_argument("--patience", type=int, default=10)
    lc_live.add_argument("--gradient-clip", type=float, default=5.0)
    lc_live.add_argument("--negbin-weight", type=float, default=1.0)
    lc_live.add_argument("--win-weight", type=float, default=0.5)
    lc_live.add_argument("--ei-weight", type=float, default=0.3)
    lc_live.add_argument("--yrfi-weight", type=float, default=0.3)
    lc_live.set_defaults(func=_cmd_learning_curves_live)
...
def _cmd_fit_pregame_full(args) -> None:
    """Train residual MLP on the full classical game_features.parquet."""
    import pandas as pd
    import torch
    from torch.utils.data import DataLoader

    from .datasets import FlatGameDataset, Standardizer, infer_feature_columns, temporal_split_dates, GAME_TARGET_COLUMNS, IDENTITY_COLUMNS
    from .distributions import gaussian_nll, weighted_mean
    from .evaluation import evaluate_pregame_model, save_evaluation
    from .models import PregameFlatModel
    ...
    log.info("Loading game_features.parquet ...")
    t = time.time()
    df = pd.read_parquet(args.game_features)
    log.info("  %d rows, %d cols (%.1fs)", len(df), len(df.columns), time.time() - t)
    ...
    # Use the classical pipeline's validated pregame allowlist to prevent leakage
    from classical_learning.strategy.data import _select_pregame_features
    feature_columns = _select_pregame_features(df)
    log.info("  %d pregame-safe feature columns (via classical allowlist)", len(feature_columns))
    ...
    train_mask = pd.to_datetime(df["game_date"]) < train_end
    standardizer = Standardizer.fit(df[train_mask], feature_columns)

    # Build datasets
    log.info("Building datasets ...")
    train_ds = FlatGameDataset(df, feature_columns, standardizer, split_end=train_end)
    val_ds = FlatGameDataset(df, feature_columns, standardizer, split_start=train_end, split_end=val_end)
    test_ds = FlatGameDataset(df, feature_columns, standardizer, split_start=val_end)
    ...
    log.info("Model: %d trainable parameters (feature_dim=%d, hidden=%d, blocks=%d)",
             n_params, len(feature_columns), args.hidden_dim, args.n_blocks)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            preds = model(batch)
            loss = _flat_model_loss(preds, batch["targets"], batch["sample_weight"])
            loss.backward()
        ...
        history.append({"epoch": epoch, "train_loss": train_loss, "val_loss": val_loss, "epoch_secs": round(epoch_time, 1)})
        ...
        mark = " * new best" if improved else f" (patience {epochs_without_improvement}/{args.patience})"
        log.info("epoch %d/%d (%.1fs) train=%.4f val=%.4f%s", epoch, args.epochs, epoch_time, train_loss, val_loss, mark)
    ...
    all_preds = {k: [] for k in ["home_win_logit", "yrfi_logit", "total_runs_mu", "total_runs_sigma", "home_run_diff_mu", "home_run_diff_sigma"]}
    all_targets = {k: [] for k in ["home_win", "yrfi", "total_runs", "home_run_diff"]}
    ...
    tr_mu = all_preds["total_runs_mu"].numpy()
    tr_actual = all_targets["total_runs"].numpy()
    rd_mu = all_preds["home_run_diff_mu"].numpy()
    rd_actual = all_targets["home_run_diff"].numpy()

    metrics = {
        "n_test": len(tr_actual),
        "home_win_brier": float(np.mean((hw_prob - hw_actual) ** 2)),
        "home_win_auc": float(_safe_auc(hw_actual, hw_prob)),
        "yrfi_brier": float(np.mean((yrfi_prob - yrfi_actual) ** 2)),
        "yrfi_auc": float(_safe_auc(yrfi_actual, yrfi_prob)),
        "total_runs_mae": float(np.mean(np.abs(tr_mu - tr_actual))),
        "total_runs_rmse": float(np.sqrt(np.mean((tr_mu - tr_actual) ** 2))),
        "home_run_diff_mae": float(np.mean(np.abs(rd_mu - rd_actual))),
        "home_run_diff_rmse": float(np.sqrt(np.mean((rd_mu - rd_actual) ** 2))),
    }

    with open(output / "test_metrics.json", "w") as f:
        json.dump(metrics, f, indent=2)

    log.info("=== Test Results (n=%d) ===", metrics["n_test"])
    log.info("  home_win:  Brier=%.4f  AUC=%.4f", metrics["home_win_brier"], metrics["home_win_auc"])
    log.info("  yrfi:      Brier=%.4f  AUC=%.4f", metrics["yrfi_brier"], metrics["yrfi_auc"])
    log.info("  total_runs: MAE=%.4f  RMSE=%.4f", metrics["total_runs_mae"], metrics["total_runs_rmse"])
    log.info("  run_diff:   MAE=%.4f  RMSE=%.4f", metrics["home_run_diff_mae"], metrics["home_run_diff_rmse"])
    log.info("Total time: %.1fs", time.time() - t0)

    # Save standardizer config for inference
    with open(output / "standardizer.json", "w") as f:
        json.dump(standardizer.to_dict(), f)

    print(json.dumps(metrics, indent=2))


def _flat_model_loss(preds: dict, targets: dict, sample_weight: torch.Tensor) -> torch.Tensor:
    """Compute multi-task loss for the flat pregame model."""
    import torch.nn.functional as F

    w = sample_weight / sample_weight.mean()

    # Binary targets
    hw_loss = F.binary_cross_entropy_with_logits(preds["home_win_logit"], targets["home_win"], weight=w, reduction="mean")
    yrfi_loss = F.binary_cross_entropy_with_logits(preds["yrfi_logit"], targets["yrfi"], weight=w, reduction="mean")

    # Gaussian NLL for continuous targets
    tr_nll = 0.5 * ((targets["total_runs"] - preds["total_runs_mu"]) / preds["total_runs_sigma"]) ** 2 + preds["total_runs_sigma"].log()
    rd_nll = 0.5 * ((targets["home_run_diff"] - preds["home_run_diff_mu"]) / preds["home_run_diff_sigma"]) ** 2 + preds["home_run_diff_sigma"].log()

    tr_loss = (tr_nll * w).mean()
    rd_loss = (rd_nll * w).mean()

    return tr_loss + rd_loss + 0.5 * hw_loss + 0.3 * yrfi_loss


def _safe_auc(y_true, y_score) -> float:
    """AUC that handles edge cases (all same class)."""
...
def _cmd_fit_live(args) -> None:
    ...
    # Free caller's reference — datasets hold their own filtered slices
    del pitch_sequences, game_targets, pregame_features
    import gc; gc.collect()
...
def _cmd_learning_curves_pregame(args) -> None:
    """Train pregame CNN at multiple data fractions to diagnose data sufficiency."""
    import pandas as pd
    import torch
    from torch.utils.data import DataLoader
    ...
    log.info("Loading data for learning curves ...")
    team_games = pd.read_parquet(feature_store / "team_games.parquet")
    game_targets = pd.read_parquet(feature_store / "game_targets.parquet")
    ...
    full_train_ds = PregameSequenceDataset(team_games, game_targets, standardizer, spec, split_end=train_end)
    val_ds = PregameSequenceDataset(team_games, game_targets, standardizer, spec, split_start=train_end, split_end=val_end)
    test_ds = PregameSequenceDataset(team_games, game_targets, standardizer, spec, split_start=val_end)

    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=4, pin_memory=True)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=0) if len(test_ds) > 0 else None

    device = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu")
    log.info("Device: %s", device)

    results = []
    full_n = len(full_train_ds)
    log.info("Full training set: %d samples", full_n)

    for frac in sorted(args.fractions):
        log.info("=== Learning curve: fraction=%.2f ===", frac)
        t_frac = time.time()

        n_subset = max(int(full_n * frac), 1)
        # Deterministic subset: take the LAST n_subset samples (most recent games)
        subset_indices = list(range(full_n - n_subset, full_n))
        subset_ds = torch.utils.data.Subset(full_train_ds, subset_indices)
        train_loader = DataLoader(subset_ds, batch_size=args.batch_size, shuffle=True, num_workers=4, pin_memory=True)
        ...
        for epoch in range(1, args.epochs + 1):
            train_loss = _run_epoch(
                ...
                gaussian_nll=gaussian_nll, weighted_mean=weighted_mean,
                epoch=epoch, total_epochs=args.epochs, n_batches=len(train_loader),
            )
            val_loss = _evaluate(
                model, val_loader, device,
                gaussian_nll=gaussian_nll, weighted_mean=weighted_mean,
                epoch=epoch, total_epochs=args.epochs, n_batches=len(val_loader),
            )
        ...
        if test_loader is not None:
            test_loss = _evaluate(
                ...
                gaussian_nll=gaussian_nll, weighted_mean=weighted_mean,
                epoch=0, total_epochs=0, n_batches=len(test_loader),
            )

        row = {
            "fraction": frac,
            "n_train_samples": n_subset,
            "best_train_loss": best_train,
            "best_val_loss": best_val,
            "test_loss": test_loss,
            "train_val_gap": best_train - best_val if best_train != float("inf") else None,
            "epochs_trained": epoch,
            "time_secs": round(time.time() - t_frac, 1),
        }
        results.append(row)
        log.info("  fraction=%.2f  n=%d  train=%.4f  val=%.4f  test=%s  gap=%.4f  (%ds)",
                 frac, n_subset, best_train, best_val,
                 f"{test_loss:.4f}" if test_loss else "N/A",
                 row["train_val_gap"] or 0, row["time_secs"])

    (output / "learning_curves_pregame.json").write_text(json.dumps(results, indent=2))
    log.info("Learning curves saved to %s", output / "learning_curves_pregame.json")
    log.info("Total runtime: %.1fs", time.time() - t0)
    ...
    if len(results) >= 2:
        ...
        val_delta = second_last["best_val_loss"] - last["best_val_loss"]
        log.info("Val loss improvement from %.0f%% to 100%%: %.4f",
                 second_last["fraction"] * 100, val_delta)
        if val_delta > 0.01:
            log.info("VERDICT: Still improving with more data → DATA-CONSTRAINED")
        elif abs(last["train_val_gap"] or 0) > 0.1:
            log.info("VERDICT: Large train/val gap → OVERFITTING (try more regularization)")
        else:
            log.info("VERDICT: Plateau → gains from architecture/features, not more data")


def _cmd_learning_curves_live(args) -> None:
    """Train live HAN at multiple data fractions to diagnose data sufficiency."""
    import pandas as pd
    import torch
    from torch.utils.data import DataLoader
    ...
    log.info("Loading data for live learning curves ...")
    pitch_sequences = pd.read_parquet(feature_store / "pitch_sequences.parquet")
    game_targets = pd.read_parquet(feature_store / "game_targets.parquet")
    ...
    # Build val/test datasets once (they don't change across fractions)
    val_ds = LiveHANDataset(
        pitch_sequences, game_targets, pregame_features, standardizer,
        split_start=train_end, split_end=val_end,
        stride=25, max_prefixes_per_game=32, max_seq_len=args.max_seq_len,
    )
    test_ds = LiveHANDataset(
        pitch_sequences, game_targets, pregame_features, standardizer,
        split_start=val_end,
    ...
    val_loader = DataLoader(val_ds, batch_size=args.batch_size, shuffle=False, num_workers=0, pin_memory=True, collate_fn=_collate_live_batch)
    test_loader = DataLoader(test_ds, batch_size=args.batch_size, shuffle=False, num_workers=0, collate_fn=_collate_live_batch) if len(test_ds) > 0 else None

    device = torch.device("cuda" if torch.cuda.is_available() else "mps" if torch.backends.mps.is_available() else "cpu")
    log.info("Device: %s", device)

    # Build full training dataset
    full_train_ds = LiveHANDataset(
        pitch_sequences, game_targets, pregame_features, standardizer,
        split_end=train_end,
        stride=25, max_prefixes_per_game=32, max_seq_len=args.max_seq_len,
    )
    ...
    full_n = len(full_train_ds)
    log.info("Full training set: %d sub-game samples", full_n)
    ...
    results = []
    for frac in sorted(args.fractions):
        log.info("=== Learning curve (live): fraction=%.2f ===", frac)
        t_frac = time.time()
        ...
        train_loader = DataLoader(
            ...
            num_workers=0, pin_memory=True, collate_fn=_collate_live_batch,
        )
        ...
        for epoch in range(1, args.epochs + 1):
            train_loss = _run_live_epoch(
                model, train_loader, optimizer, device, loss_config, args.gradient_clip,
                epoch=epoch, total_epochs=args.epochs, n_batches=len(train_loader),
            )
            val_loss = _evaluate_live(
                model, val_loader, device, loss_config,
                epoch=epoch, total_epochs=args.epochs, n_batches=len(val_loader),
            )
        ...
        test_loss = None
        if test_loader is not None:
            test_loss = _evaluate_live(
                model, test_loader, device, loss_config,
                epoch=0, total_epochs=0, n_batches=len(test_loader),
            )
    ...
    (output / "learning_curves_live.json").write_text(json.dumps(results, indent=2))
    log.info("Learning curves saved to %s", output / "learning_curves_live.json")
    log.info("Total runtime: %.1fs", time.time() - t0)
