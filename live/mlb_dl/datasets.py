...
class PregameSequenceDataset:
    ...
    def _team_sequence(self, team_id, game_date):
        ...
        pad_count = self.spec.history_length - n_prior
        if pad_count > 0:
            ...
            padding = np.concatenate(
                [np.zeros(pad_count, dtype="float32"), np.ones(n_prior, dtype="float32")]
            )
        else:
            padding = np.ones(self.spec.history_length, dtype="float32")

        return values, mask, padding
...
def temporal_split_dates(game_targets, train_fraction=0.80, val_fraction=0.10):
    import pandas as pd

    dates = (
        pd.to_datetime(game_targets["game_date"], errors="coerce")
        .dropna()
        .sort_values()
        .drop_duplicates()
        .to_list()
    ...
    if len(dates) < 10:
        raise ValueError("Need at least 10 distinct game dates for temporal train/val/test split")
