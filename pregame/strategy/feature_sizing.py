...
from .config import IMPORTANCE_DIR, NEEDS_IMPUTATION, TARGETS_CLASSIFICATION
...
def _build_fixed_model(family: str, task: str):
    ...
    # Linear/fragile models — use HistGradientBoosting as proxy since these
    # models get very few features from routing anyway (3-10). Their sizing
    # is dominated by routing, not the cap.
    if task == "classification":
        return HistGradientBoostingClassifier(
            max_iter=300, max_leaf_nodes=31, learning_rate=0.05,
            min_samples_leaf=50, l2_regularization=1.0,
...
def run_sizing_curve(
    ...
    for family in ALL_FAMILIES:
        ...
        for n_feat in cutoffs:
            ...
            X_tr = X.iloc[sizing_split.train_idx][selected]
            X_va = X.iloc[sizing_split.val_idx][selected]
            ...
            try:
                if hasattr(model, "fit") and "sample_weight" in str(type(model).fit.__code__.co_varnames):
                    model.fit(X_tr, y_train, sample_weight=sample_weights.values)
                else:
            ...
            val_loss = _compute_metrics_simple(y_val.values, val_preds, task)
            curve.append({"n_features": n_feat, "val_loss": round(val_loss, 6)})
        ...
        if fine_grained and family_max > 20:
            ...
            for n_feat in fine_range:
                ...
                if task == "classification":
                    val_preds = model.predict_proba(X_va)[:, 1] if hasattr(model, "predict_proba") else model.predict(X_va)
                else:
                    val_preds = model.predict(X_va)
        ...
        per_family_results[family] = {
            ...
            "all_features_val_loss": curve[-1]["val_loss"],
