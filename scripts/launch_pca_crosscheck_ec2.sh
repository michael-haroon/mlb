...
CODE_KEY="artifacts/code/mlb_pca_crosscheck.tar.gz"
...
# Run PCA cross-check
python3.11 scripts/run_pca_crosscheck_ec2.py --target ${TARGET}
EXIT_CODE=\$?

echo "=== DONE: ${TARGET} exit_code=\${EXIT_CODE} at \$(date -u) ==="

# Upload log to S3
aws s3 cp "\$LOG_FILE" s3://${BUCKET}/artifacts/importance/${TARGET}/pca_crosscheck.log
...
USERDATA_EOF
    ...
    INSTANCE_ID=$(aws ec2 run-instances \
        ...
        --tag-specifications "ResourceType=instance,Tags=[{Key=Name,Value=pca_crosscheck_${TARGET}},{Key=Purpose,Value=pca_crosscheck_v2}]" \
        --user-data "$USER_DATA" \
...
echo "  aws ec2 describe-instances --filters 'Name=tag:Purpose,Values=pca_crosscheck_v2' --query \"Reservations[*].Instances[*].{ID:InstanceId,Name:Tags[?Key=='Name']|[0].Value,State:State.Name}\" --output table"
echo ""
echo "Logs upload to: s3://${BUCKET}/artifacts/importance/<target>/pca_crosscheck.log"
echo "Results at:     s3://${BUCKET}/artifacts/importance/<target>/kendall_tau.json"
