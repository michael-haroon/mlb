#!/bin/bash
# BATCH 2: Remaining 76 instances that didn't launch due to vCPU cap.
# Launches: home_run_diff (desub_mda, pca_mda, resid_mda) + 5 full targets
...
# Remaining combos:
# home_run_diff: desub_mda, pca_mda, resid_mda (6 instances)
# Then full targets: total_runs, home_runs, away_runs, first_5_home_run_diff, first_5_total_runs (70 instances)
...
echo "=== BATCH 2: Launching remaining 76 importance instances ==="
echo ""

# home_run_diff remainder
for TEST in desub_mda pca_mda resid_mda; do
    for CV_MODE in "${CV_MODES[@]}"; do
        launch_test home_run_diff "$TEST" "$CV_MODE"
    done
done

# Full remaining targets
for TARGET in total_runs home_runs away_runs first_5_home_run_diff first_5_total_runs; do
...
echo ""
echo "=== BATCH 2: ${LAUNCHED} instances launched ==="
echo ""
echo "Monitor:"
