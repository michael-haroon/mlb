"""Run ONE importance test for ONE target on EC2 — RATINGS SUBSET ONLY.

Identical to run_importance_single_test_ec2.py except features are filtered
to only rating/ranking systems (elo, massey, colley, wolfe, pythag, srs,
log5, consensus) and their interactions. These are features that require
solving systems of equations — not learnable by deep learning.

Usage:
    python3.11 scripts/run_importance_ratings_ec2.py --target home_win --test mda --cv-mode expanding
"""
...
# Rating system keywords — any feature column containing one of these (excluding
# 'velo' false positives) is a rating/ranking feature that requires solving
# systems of equations, iterative methods, or complex mathematical derivations.
...
def main():
    ...
    log.info(f"{'='*70}")
    log.info(f"RATINGS SUBSET | Target: {target} | Test: {test_name} | Task: {task} | CV: {cv_mode}")
    log.info(f"{'='*70}")
    ...
    # Clustering (target-independent) — fast at 59 features
    if cluster_key:
        clusters = download_cluster_map(s3, cluster_key)
    else:
        log.info("Computing ONC clustering on ratings subset...")
        ...
        upload_artifacts(s3, target, test_name, cluster_artifacts, cv_mode=cv_mode)
