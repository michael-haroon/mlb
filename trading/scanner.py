...
2. Fair value (clipped model probability)
...
from .config import (
    HALF_SPREAD_BASE_CENTS, PRICE_FLOOR, PRICE_CEILING,
    MIN_EDGE_BUFFER_MAKER,
)
from .targets import TOTAL_RUNS_CONFIG
...
def conservative_fair_value(model_prob: float) -> float:
    """Fair value from model probability. Clipped to avoid degenerate quotes."""
...
def _price_market(
    ...
    fair = conservative_fair_value(model_prob)

    # Price filter: skip extreme prices where we have no edge
    if fair < PRICE_FLOOR or fair > PRICE_CEILING:
        return None

    # NegBin-derived adaptive spread for total_runs; fixed for other targets
    if target == "total_runs" and "point_estimate" in result:
        mu = result["point_estimate"]
        alpha = base_result.get("distribution", {}).get("alpha", TOTAL_RUNS_CONFIG.negbin_alpha)
        half_spread = TOTAL_RUNS_CONFIG.get_half_spread(mu, alpha)
    else:
        half_spread = HALF_SPREAD_BASE_CENTS
